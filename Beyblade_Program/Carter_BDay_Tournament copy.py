...
class TournamentApp:
    ...
    def generate_name_fields(self):
        ...
        if count not in [4, 8, 10, 16, 18, 20, 32]:
            messagebox.showerror("Invalid Number", "Please enter a valid number of participants (4, 8, 16, 18, 20, 32).")
            return
    ...
    def show_bracket(self):
        ...
        # Create buttons and pass both to select_winner so we can disable both after selection
            b1 = ttk.Button(match_frame, text=p1, command=lambda name=p1, btns=None: self.select_winner(name, b1, b2))
            b2 = ttk.Button(match_frame, text=p2, command=lambda name=p2, btns=None: self.select_winner(name, b1, b2))
    ...
    def select_winner(self, name, btn1, btn2):
        ...
        if len(self.next_round_players) == len(self.players) // 2:
            ...
            self.players = self.next_round_players[:]  # copy winners to players
            self.next_round_players.clear()            # reset for next round
            self.current_round += 1
    ...
    def show_winner(self, name):
        self.bracket_frame.destroy()
        winner_label = tk.Label(self.canvas, text=f"🏆 {name} is the Champion! 🏆", font=("Arial", 24, "bold"), bg="gold", fg="black")
