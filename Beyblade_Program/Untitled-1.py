...
class TournamentAppWithBackground:
    def __init__(self, root):
        ...
        image_path= PhotoImage(file= r"C:\Users\Alex\Documents\Beyblade_Program\images\beyblade_emblem.png" )

        bey_canvas = Canvas(root, width= 750, height= 500)
        bey_canvas.pack(fill="both", expand= "True")

        bey_canvas.create_image(0,0, image= image_path, anchor= "nw")

        bey_canvas.create_text(350,250, text= "Carter's Birthday Tournament",font=("Helvetica",50) ,fill= "red", anchor= "nw")

        
        #self.canvas = tkinter.Canvas(self.root, width=1000, height=700)
        #self.canvas.pack(fill="both", expand=True)
        #self.canvas.create_image(0, 0, image=self.bg_photo, anchor="nw")
        
        # You will dynamically add widgets on top of this canvas
        self.num_var = tkinter.StringVar()
        self.setup_participant_number_selection()

    def setup_participant_number_selection(self):
        # Label
        label = tkinter.Label(self.root, text="Select number of participants:", font=("Arial", 14), background="#ffffff")
        self.canvas.create_window(500, 100, window=label)

        # Dropdown
        participant_options = ["2", "4", "8", "16"]
        num_select = tkinter.Combobox(self.root, textvariable=self.num_var, values=participant_options, state="readonly")
        self.canvas.create_window(500, 140, window=num_select)

        # Button
        start_button = tkinter.Button(self.root, text="Next", command=self.setup_name_entry)
    ...
    def setup_name_entry(self):
        ...
        for i in range(self.num_participants):
            label = tkinter.Label(self.root, text=f"Player {i+1}:", background="#ffffff")
        ...
        start_button = tkinter.Button(self.root, text="Start Tournament", command=self.start_tournament)
...
class TournamentBracket:
    ...
    def build_round(self, current_players, round_number, x_pos):
        ...
        for i in range(0, len(current_players), 2):
            ...
            label = tkinter.Label(self.root, text=f"{p1} vs {p2}", background="#ffffff")
            self.canvas.create_window(x_pos, y, window=label)

            winner_select = tkinter.Combobox(self.root, values=[p1, p2], state="readonly")
        ...
        submit_button = tkinter.Button(self.root, text="Submit Round", command=lambda: self.collect_winners(round_widgets, round_number + 1, x_pos + 200))
